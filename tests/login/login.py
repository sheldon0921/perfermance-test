...
class Login():
    ...
    def loginShop():
        ...
        res = Login.loginNew(enterpriseID)
        logger.info("login res:{0}".format(res))
        ...
        if res['message'] == 'success':
            try:
                ...
                raise Exception("Login shop fail, get token fail!", res)
        else:
            raise Exception("Login shop fail! return code is {0}".format(res['code']), res)
    ...
    def loginConsole():
        ...
        account = ReaderIniFile.value(section="console_sys_info", key="consoleAccount")
        password = ReaderIniFile.value(section="console_sys_info", key="consolePassword")
        loginType = ReaderIniFile.value(section="console_sys_info", key="loginType")
        enterpriseHash = ReaderIniFile.value(key="enterpriseHash")
        # 登录接口参数拼接
