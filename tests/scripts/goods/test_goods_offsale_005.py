...
class Test_goods_add_publish_onsale_goods(object):
    ...
    def ini(self):
        ...
        yield self.offsaleParam, self.spuId,self.shopGoods
    ...
    @allure.story('新增并上架单sku商品，可以下架成功，且小程序端搜不到该商品')
    @pytest.mark.smoke
    def test_goods_offsaleGoods(self, ini):
        ...
        with allure.step(f"通过 spu: {self.spuId} 查询待上架列表第一条数据"):
            assert goodlistRes["data"]["list"][0]["spu_id"] == self.spuId
        # 在小程序端搜索不到到此商品
        spuName = goodlistRes["data"]["list"][0]["sku_name"]
        searchResult = self.shopGoods.queryGoodsByGoodsName(spuName)
        with allure.step(f"通过 spu_name: {spuName} 查询小程序端商品"):
        ...
        if "sku" in self.listData:
            ...
            self.postageIdList.append(postageId)
        self.postageIdList = list(set(self.postageIdList))

        print("22222222222:    ",self.postageIdList)
