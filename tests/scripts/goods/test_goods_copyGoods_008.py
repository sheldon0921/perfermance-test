...
class Test_goods_copyGoods(object):
    ...
    def ini(self):
        ...
        goodlistParam = {"searchType": 1, "searchParams": {"searchString": {"keyword": str(self.spuId), "type": 4},
                         ...
                         "size": 10}
        with allure.step(f"在sku列表查询 spu为 {self.spuId} 的商品 {self.addGoodsParam['spu_name']} "):
        ...
        yield self.skuIds,self.addPostageParam, self.addGoodsParam, self.addSpecTemplateParam, self.spuId
    ...
    @allure.story('复制并保存待上架的多sku商品，且在待上架列表可以查到该商品')
    @pytest.mark.smoke
    def test_goods_copyGoods(self, ini):
        # 复制全部spu列表-待上架商品
        self.copySpuId = []
        for i in self.skuIds:
            param = {"spu_id": self.spuId,"sku_id":i}
            ...
            self.copySpuId.append(saveRes["data"]['spu_id'])
