...
class Test_goods_publish_noSale(object):
    ...
    @allure.story('发布多sku的商品，断言商品sku图及状态')
    @pytest.mark.smoke
    def test_goods_publish_noSale(self, ini):
        ...
        status = goodsInfo["status"]["valueString"]
        with allure.step(f"查询 spu为 {self.spuId} 的商品sku 状态为 {status} "):
            assert status == "待上架"
            assert goodsInfo["spu_id"] == self.spuId
            assert goodsInfo["sku_image"] == "https://img-crs.vchangyi.com/goods161165268167619.png"
        # 查询商品关联的规格模板
