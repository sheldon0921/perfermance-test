...
class Test_goods_copyGoods(object):
    ...
    def ini(self):
        ...
        with allure.step(f"通过复制的 copyspu: {self.copySpuId} 删除商品 {self.addGoodsParam['spu_name']} "):
            delRes = self.goods.delSpu(delGoods).json()
    ...
    @allure.story('复制并保存未发布的单sku商品，且在未发布列表可以查到该商品')
    @pytest.mark.smoke
    def test_goods_copyGoods(self, ini):
        ...
        listParam = {"page": 1, "size": 10, "searchParams": {"searchChannel": 0, "searchChannelOwnership": 0,
                                                             "searchString": {"keyword": str(self.copySpuId),
                                                                              "type": 4},
        ...
        with allure.step(f"在未发布商品列表查询 copyspu为 {self.copySpuId} 的商品 {self.addGoodsParam['spu_name']} "):
            listRes = self.goods.listSpu(listParam)
            assert listRes["data"]["list"][0]["id"] == self.copySpuId
        self.logger.info("listRes: {0}".format(listRes))
        print('########{0}'.format(listRes))
