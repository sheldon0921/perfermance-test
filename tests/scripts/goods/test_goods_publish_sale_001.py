...
class Test_goods_publish_noSale(object):
    ...
    @allure.story('上架库存不足的单sku商品，上架失败')
    @pytest.mark.smoke
    def test_goods_publish_Sale(self, ini):
        # 保存商品信息
        ...
        # 2021/7/6和李坚确认，商品库存与物料库存已经无关，物料库存后期会去掉；
        with allure.step(f"修改库存数量"):
            assert updateExpressRes["message"] == "success"
        saleParam = {"spu_id":[self.spuId],"sku_id":[]}
        saleRes = self.goods.saleGoods(saleParam)
        self.logger.info("saleRes:{0}".format(saleRes))
        with allure.step(f"商品上架成功"):
            assert saleRes["data"]["success_num"] == 1
        # 查询商品关联的规格模板
