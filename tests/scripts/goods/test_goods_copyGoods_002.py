...
class Test_goods_copyGoods(object):
    ...
    def ini(self):
        ...
        # 查询未发布商品列表的该spu数据
        listParam = {"page": 1, "size": 10, "searchParams": {"searchChannel": 0, "searchChannelOwnership": 0,
                                                             "searchString": {"keyword": str(self.spuId), "type": 4},
        ...
        with allure.step(f"在待上架商品列表查询 spu为 {self.spuId} 的商品 {self.addGoodsParam['spu_name']} "):
        ...
        with allure.step(f"在全部商品列表查询 spu为 {self.spuId} 的商品 {self.addGoodsParam['spu_name']} "):
            listRes = self.goods.listSpu(listParam)
            assert listRes["data"]["list"][0]["id"] == self.spuId
            assert listRes["data"]["list"][0]["status"]["value"] == 1
        self.logger.info("listRes: {0}".format(listRes))
    ...
    @allure.story('复制并保存待上架的单sku商品，且在待上架列表可以查到该商品')
    @pytest.mark.smoke
    def test_goods_copyGoods(self, ini):
        ...
        # 查询未发布商品列表可查到该复制的spuid
        listParam = {"page": 1, "size": 10, "searchParams": {"searchChannel": 0, "searchChannelOwnership": 0,
                                                             "searchString": {"keyword": str(self.copySpuId), "type": 4},
                                                             "searchInt": {"high": 0, "low": 0, "type": 1},
                                                             "searchNeedDelete": 2,
                                                             "searchTime": {"beginTimestamp": 0, "endTimestamp": 0,
                                                                            "type": 1}, "searchGoodsStatus": -1,
                                                             "searchReleaseStatus": 0,
                                                             "searchSort": {"sortColumn": 0, "sortType": 0}}}
        with allure.step(f"在待上架商品列表查询 copyspu为 {self.copySpuId} 的商品 {self.addGoodsParam['spu_name']} "):
            listRes = self.goods.listSpu(listParam)
            assert listRes["data"]["list"][0]["id"] == self.copySpuId
        self.logger.info("listRes: {0}".format(listRes))
