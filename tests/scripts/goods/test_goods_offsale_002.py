...
class Test_goods_add_publish_onsale_goods(object):
    ...
    @allure.story('新增并上架单sku商品，可以下架成功，且为全部列表第一条数据，状态为待上架')
    @pytest.mark.smoke
    def test_goods_offsaleGoods(self, ini):
        ...
        # 在待上架列表中查询此商品，应为第一条数据，查询到即说明下架成功
        # 全部spu列表的第一条数据
        # 商品状态：待上架
        listParam = {"page": 1, "size": 1, "searchParams": {"searchString":{"keyword":str(self.spuId),"type":4},"searchReleaseStatus": 1}}
        ...
        with allure.step(f"通过 spu: {self.spuId} 查询全部spu列表第一条数据，且状态为 {goodstatus}"):
