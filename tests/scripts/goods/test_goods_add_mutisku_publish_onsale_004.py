...
@allure.feature('商品')
class Test_goods_add_mutiSku_publish_004(object):
    ...
    def ini(self):
        ...
        yield self.saleParam, self.addPostageParam, self.addSpecTemplateParam, self.addGoodsParam, self.specTemplate
        # 下架商品
    ...
    @allure.story('添加多sku的商品,上架商品，小程序可以搜到该商品')
    @pytest.mark.smoke
    def test_goods_add_mutiSku_publish(self, ini):
        ...
        # 查询普通商品列表的前两条数据
        goodlistParam = {"searchType": 1, "searchParams": {"searchString": {"keyword": str(self.spuId), "type": 4},
        ...
        for i in goodlistRes["data"]["list"]:
            self.logger.info("i:                  {0}".format(i))
            if i["spu_id"] == self.spuId:
                self.skuIds.append(i["sku_id"])
            # 在小程序端可查询到此商品

                skuName = i["sku_name"]
        print("skuname%%%%    :{0}".format(skuName))
        with allure.step(f"小程序端商品为  {skuName}"):
            searchResult = self.shopGoods.queryGoodsByGoodsName(skuName)
            self.logger.info("searchResult： {0}".format(searchResult))
        count = 1
        while len(searchResult["data"]["list"]) == 0:
            ...
            if count == 131:
                break
        print("len: {0}".format(len(searchResult["data"]["list"])))
