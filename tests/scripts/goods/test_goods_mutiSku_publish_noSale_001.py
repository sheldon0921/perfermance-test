...
class Test_goods_publish_noSale(object):
    ...
    @allure.story('添加发布多sku商品，为待上架列表第一条，且为待上架状态')
    @pytest.mark.smoke
    def test_goods_publish_noSale(self, ini):
        ...
        resSpuId = goodsList["data"]["list"][0]["spu_id"]
        self.logger.info("spuId: {0}".format(resSpuId))
        assert self.spuId == resSpuId
        goodsStatus = goodsList["data"]["list"][0]["status"]["valueString"]
        self.logger.info("goods status: {0}".format(goodsStatus))
