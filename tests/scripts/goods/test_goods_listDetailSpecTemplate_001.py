...
class Test_goods_listDetailSpecTemplate(object):
    ...
    def ini(self):
        ...
        yield self.specTemplate, self.addParam, self.logger, self.templateId
        # 删除规格模板
    ...
    @allure.story('新增规格模板成功，且在规格模板页正确展示')
    @pytest.mark.smoke
    def test_goods_updateSpecTemplate(self, ini):
        listDetailParam = {"attribute_template_id": self.templateId}
        listDetailRes = self.specTemplate.listDetailSpecTemplate(listDetailParam)
        self.logger.info("listDetailRes: {0}".format(listDetailRes))
        with allure.step(f"通过规格模板ID : {self.templateId} 查询规格模板 "):
            assert "success" == listDetailRes["message"]
            listDetailData = listDetailRes["data"]
            assert self.templateId == listDetailRes["data"]["id"]
            assert listDetailData["created_at"] == listDetailData["updated_at"]
            assert self.addParam["template_name"] == listDetailData["template_name"]
        # print("listDetailRes: {0}".format(listDetailRes))
