...
class Test_goods_publish_noSale(object):
    ...
    @allure.story('添加多sku商品，为未发布列表第一条，发布后发布成功')
    @pytest.mark.smoke
    def test_goods_publish_noSale(self, ini):
        ...
        # 未发布商品的第一条数据
        goodsListParam = {"page": 1, "size": 10, "searchParams": {"searchChannel": 0, "searchChannelOwnership": 0,
                                                                  "searchString": {"keyword": str(self.spuId),
                                                                                   "type": 4},
        ...
        noPublishGoodsRes = self.goods.listSpu(goodsListParam)
        print('########1:{0}'.format(noPublishGoodsRes))
        with allure.step(f"在未发布列表查询 spu为 {self.spuId} 的商品 "):
            assert self.spuId == noPublishGoodsRes["data"]["list"][0]["id"]
        ...
        with allure.step(f"商品 {self.addGoodsParam['spu_name']} 状态为 {goodsList['data']['list'][0]['status']['valueString']} "):
            assert self.spuId == goodsList["data"]["list"][0]["spu_id"]
            assert "待上架" == goodsList["data"]["list"][0]["status"]["valueString"]
