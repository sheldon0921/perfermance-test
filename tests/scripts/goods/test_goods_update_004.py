...
class Test_goods_updateGoods_002(object):
    ...
    def ini(self):
        ...
        yield self.publishGoodsParam,self.spuId,self.useOneSpecTemplate,self.skuParams
    ...
    @allure.story('已有商品修改商品的sku信息')
    @pytest.mark.smoke
    def test_goods_publishGoods(self, ini):
        # 修改sku规格
        self.skuParams[0]["attribute_item_ids"] = self.useOneSpecTemplate
        # 修改sku划线价，库存，图片,售卖价
        self.skuParams[0]["sign_price"] = 9999
        self.skuParams[0]["on_line_price"] = 7777
        self.skuParams[0]["on_line_current_stock"] = 88
        self.skuParams[0]["resource"]["sku_picture"][0]["url"] = "https://img-crs.vchangyi.com/goods16161339182230.jpg"
        ...
        with allure.step(f"通过商品spu: {self.spuId} 查询其sku信息"):
            ...
            assert res["sku_image"] == self.skuParams[0]["resource"]["sku_picture"][0]["url"]
            assert res["price"] == self.skuParams[0]["on_line_price"]
            assert res["stock"]["online_stock"] == self.skuParams[0]["on_line_current_stock"]
            assert res["attributes"][1]["attribute_value_id"] == self.useOneSpecTemplate[1] or res["attributes"][0][
                "attribute_value_id"] == self.useOneSpecTemplate[1]
            assert res["attributes"][0]["attribute_value_id"] == self.useOneSpecTemplate[0] or res["attributes"][0]["attribute_value_id"] == self.useOneSpecTemplate[1]
