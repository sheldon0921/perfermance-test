...
class Test_goods_publish_noSale(object):
    ...
    @allure.story('发布多sku的商品，断言商品主图及状态')
    @pytest.mark.smoke
    def test_goods_publish_noSale(self, ini):
        ...
        goodsListSpuParam = {"page": 1, "size": 1, "searchParams": {"searchString":{"keyword":str(self.spuId),"type":4},"searchReleaseStatus": 1}}
        listSpuRes = self.goods.listSpu(goodsListSpuParam)
        ...
        with allure.step(f"查询 spu为 {self.spuId} 的商品 状态为 {status} "):
            assert status == "待上架"
            assert goodsInfo["id"] == self.spuId
            assert goodsInfo["picture"] == "https://img-crs.vchangyi.com/goods161165268167617.png"
        # 查询skuID
        goodsListParam = {"searchType":1,"searchParams":{"searchString":{"keyword":str(self.spuId),"type":4},"searchInt":{"high":0,"low":0,"type":1},"searchChannel":0,"searchTime":{"beginTimestamp":0,"endTimestamp":0,"type":1},"searchGoodsStatus":0,"searchChannelOwnership":0,"searchSort":{"sortColumn":0,"sortType":0}},"page":1,"size":10}
        goodsList = self.goods.queryGoodsList(goodsListParam)
        self.logger.info("goodsListRes: {0}".format(goodsList))
        self.skuIds = [goodsList["data"]["list"][0]["sku_id"], goodsList["data"]["list"][1]["sku_id"]]
        # 查询商品关联的规格模板
