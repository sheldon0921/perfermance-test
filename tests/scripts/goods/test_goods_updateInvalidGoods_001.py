...
@allure.feature('商品')
class Test_goods_updateInvalidGoods_001(object):
    @pytest.fixture()
    def ini(self):
        self.goods = Goods()
    ...
    @allure.story('已失效商品修改库存，提示无渠道信息')
    @pytest.mark.smoke
    def test_goods_updateInvalidGoods(self, ini):
        # 保存商品信息
        self.addGoodsParam['sku'][0]['material_id'] = 162336125530688
        self.addGoodsParam['sku'][0]['material_no'] = 'No0001'
        ...
        goodsListParam = {"searchType":4,"searchParams":{"searchString":{"keyword":str(self.spuId),"type":4},"searchInt":{"high":0,"low":0,"type":1},"searchChannel":0,"searchTime":{"beginTimestamp":0,"endTimestamp":0,"type":1},"searchGoodsStatus":0,"searchChannelOwnership":0,"searchSort":{"sortColumn":0,"sortType":0}},"page":1,"size":10}
        ...
        stockParam = {"page": 1, "size": 10, "first_value": "No0001", "first_type": "material_no",
                      "second_type": "all", "min_value": "", "max_value": "", "source_id": 0, "time_type": "create",
                      "start_time": "", "end_time": ""}
        stockListRes = self.stock.queryStockList(stockParam)
        self.logger.info("stockListRes:{0}".format(stockListRes))
        totalStock = stockListRes[0]["total_stock"]
        # 修改库存数量
        updateExpress = {"source_id": 133719584357376, "type": "3", "online": 1, "number": totalStock+1, "sku_id": self.skuId}
        updateExpressRes = self.stock.updateChannelStock(updateExpress)
        self.logger.info("updateExpressRes:{0}".format(updateExpressRes))
        #查询库存数量
        stockParam = {"page": 1, "size": 10, "first_value": "No0001", "first_type": "material_no",
                      "second_type": "all", "min_value": "", "max_value": "", "source_id": 0, "time_type": "create",
                      "start_time": "", "end_time": ""}
        stockListRes = self.stock.queryStockList(stockParam)
        self.logger.info("stockListRes1:{0}".format(stockListRes))
        totalStock1 = stockListRes[0]["total_stock"]
        with allure.step(f"通过 sku {self.skuId} 修改库存量 "):
            assert totalStock1 == totalStock
        # 查询商品关联的规格模板
