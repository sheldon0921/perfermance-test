...
class Test_Placeorder(object):
    ...
    def ini(self):
        ...
        #查询sku_id
        queryparam = {"searchType":2,"searchParams":{"searchString":{"keyword":str(self.spuid),"type":4},"searchInt":{"high":0,"low":0,"type":1},"searchChannel":0,"searchTime":{"beginTimestamp":0,"endTimestamp":0,"type":1},"searchGoodsStatus":0,"searchChannelOwnership":0,"searchSort":{"sortColumn":0,"sortType":0}},"page":1,"size":10}
        ...
        # 添加用户地址
        addAddressRes = self.goods.addAddress(self.addAddressParam)
        ...
        deleteAddressRes=self.goods.deleteAddress(deleteAddressparam)
    ...
    @allure.story('用户直接下单')
    @pytest.mark.smoke
    def test_confirmOrder(self,ini):
        #获取邮费
        getpostageparam={"sku_id":self.postageid,
                         "activity_id":"",
                         "module":"default"
                         }
        getpostageRes=self.goods.getpostAge(getpostageparam)
        self.log.Logger().info('getpostageRes:{0}'.format(getpostageRes))
        with allure.step(f"获取邮费列表"):
            assert getpostageRes['code'] == 200 and getpostageRes['message'] == 'success'
        #确认订单
        orderparam={"goods_data":[{"sku_id":self.skuId,"number":1,"cart_id":0,"delivery_type":1}],"user_address_id":self.addressId}
        confirmOrderRes=self.goods.confirmOrder(orderparam)
        ...
        placeOrderRes = self.goods.placeOrder(self.placeOrderParam)
        ...
        queryOrderRes = self.goods.queryOrderlist(queryOrderparam)
        ...
        with allure.step(f"查询后腰全部订单，状态为：待支付"):
        ...
        id = self.goods.querySuborder(queryOrderRes, orderNo)

        # 取消订单
        cancelOrderparam = {"id": id, "event": "customer_cancel", "refund_reason_id": 0}
        cancelOrderRes = self.goods.cancelOrder(cancelOrderparam)
        ...
        '''
        orderDetailparam = {"order_id": '', "main_order_no": orderNo}
        orderDetailRes = self.goods.orderDetail(orderDetailparam)
        self.log.Logger().info('orderDetailRes:{0}'.format(orderDetailRes))
        with allure.step(f"查询小程序端订单状态为：已取消"):
