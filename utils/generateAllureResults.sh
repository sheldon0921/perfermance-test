...
if ['postman', 'unknown', 'local', ''].include?(test_type)
  Dir.glob(File.join(newman_xml_dir, '*.xml')).sort.each do |xml_path|
    begin
      ...
      warnings << "Skipped Newman XML #{xml_path}: #{e.class}: #{e.message}"
    end
  end
end
...
if ['jmeter', 'unknown', 'local', ''].include?(test_type) && File.exist?(jmeter_jtl_file)
  begin
    CSV.foreach(jmeter_jtl_file, headers: true).with_index do |row, index|
      jmeter_total_rows += 1
      next if jmeter_sample_limit.positive? && jmeter_written_rows >= jmeter_sample_limit
    ...
    warnings << "Skipped JMeter JTL #{jmeter_jtl_file}: #{e.class}: #{e.message}"
  end
end
...
if File.exist?(jmeter_stats_file)
  begin
    ...
    warnings << "Skipped JMeter statistics #{jmeter_stats_file}: #{e.class}: #{e.message}"
  end
end
...
environment['Warnings'] = warnings.join(' | ') if warnings.any?
File.write(File.join(allure_dir, 'environment.properties'), environment.map { |k, v| "#{k}=#{v}" }.join("\n"))

if warnings.any?
  File.write(File.join(allure_dir, 'warnings.txt'), warnings.join("\n"))
  warn warnings.join("\n")
end
