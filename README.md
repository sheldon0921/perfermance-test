...
`JenkinsfileForLocal` 会优先通过 HTML Publisher 在构建页展示报告入口，并同时归档为 Jenkins artifacts。建议在 Jenkins 插件管理中安装 `HTML Publisher` 插件；没有该插件时，流水线会跳过页面入口发布，但构建不会因此失败，报告仍可从 artifacts 打开。
...
如果只看到“上次成功的成品”，说明 Jenkins 已经归档报告，但没有在任务页直接渲染 HTML 报告入口。安装 HTML Publisher 并使用当前 Jenkinsfile 后，构建详情页会出现 `Report Index`、`Allure Report`，以及按类型出现的 `JMeter Dashboard` 链接；未安装插件时，控制台会提示跳过 HTML 发布。
